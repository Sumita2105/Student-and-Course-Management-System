...
public class AdminController {
    ...
    public ModelAndView displayClasses(Model model) {
        List<InstituteClass> instituteClasses = instituteClassRepository.findAll();
        ModelAndView modelAndView = new ModelAndView("classes.html");
        modelAndView.addObject("eazyClasses", instituteClasses);
        modelAndView.addObject("eazyClass", new InstituteClass());
        return modelAndView;
    }

    @PostMapping("/addNewClass")
    public ModelAndView addNewClass(Model model, @ModelAttribute("eazyClass") InstituteClass instituteClass){
    ...
    public ModelAndView deleteClass(Model model, @RequestParam int id){
        ...
        Optional<InstituteClass> eazyClass = instituteClassRepository.findById(id);
        for (Person person : eazyClass.get().getPersons()){
            person.setEazyClass(null);
            personRepository.save(person);
        }

        instituteClassRepository.deleteById(id);
        ModelAndView modelAndView = new ModelAndView("redirect:/admin/displayClasses");
        return modelAndView;
    }
                                        ...
                                        @RequestParam(value="error", required = false) String error)
    {
        ...
        Optional<InstituteClass> eazyClass = instituteClassRepository.findById(classId);
        modelAndView.addObject("eazyClass", eazyClass.get());
        modelAndView.addObject("person", new Person());
        session.setAttribute("eazyClass", eazyClass.get());
        if(error!=null){
            errorMessage ="Invalid email entered";
            return modelAndView.addObject("errorMessage", errorMessage);
        }
        return modelAndView;
    }
    ...
    public ModelAndView addStudent(Model model, @ModelAttribute("person") Person person, HttpSession session) {
        ...
        InstituteClass instituteClass = (InstituteClass) session.getAttribute("eazyClass");
        Person personEntity = personRepository.readByEmail(person.getEmail());
        if(personEntity==null || !(personEntity.getPersonId()>0)){
            modelAndView.setViewName("redirect:/admin/displayStudents?classId="+ instituteClass.getClassId()
                    +"&error=true");
            return modelAndView;
        }
        personEntity.setEazyClass(instituteClass);
        personRepository.save(personEntity);
        instituteClass.getPersons().add(personEntity);
        instituteClassRepository.save(instituteClass);
        modelAndView.setViewName("redirect:/admin/displayStudents?classId="+ instituteClass.getClassId());
        return modelAndView;
    }
    ...
    public ModelAndView deleteStudent(Model model, @RequestParam int personId, HttpSession session) {
        ...
        instituteClass.getPersons().remove(person.get());
        InstituteClass instituteClassSaved = instituteClassRepository.save(instituteClass);
        session.setAttribute("eazyClass", instituteClassSaved);
        ModelAndView modelAndView = new ModelAndView("redirect:/admin/displayStudents?classId="+ instituteClass.getClassId());
        return modelAndView;
    }
